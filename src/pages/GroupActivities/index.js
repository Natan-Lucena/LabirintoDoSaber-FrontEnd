...
function GroupActivitiesPage() {
  ...
  const removeGroupActivityRow = (uniqueId) => {
    if (groupActivities.length > 1) {
      setGroupActivities(
        groupActivities.filter((item) => item.uniqueId !== uniqueId)
      );
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/task-group/create`,
        payload,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
        ...
        <div className="adicionar-atividade-container">
          ...
          <div className="form-group">
            <div className="form-label required">
              Adicione as atividades desejadas
            </div>
