...
function SessionNotebookPage() {
  ...
  const handleStartSession = async () => {
    ...
    if (targetTasksIds.length === 0) {
      alert(
        "Este caderno está vazio (não contém grupos com atividades nem atividades soltas)."
      );
      setIsStarting(false);
      return;
    }
    ...
    const resolvedTasks = targetTasksIds
      .map((item) => {
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: "1rem",
                  cursor:
                    selectedNotebook && !isStarting ? "pointer" : "not-allowed",
                  boxShadow: selectedNotebook
          ...
          <div className="session-notebook-select-card-list">
            {loading ? (
              ...
              currentItems.map((notebook) => {
                const isSelected =
                  selectedNotebook && selectedNotebook.id === notebook.id;

                return (
                  <div
                    className="notebook-select-row-wrapper"
                    key={notebook.id}
                  >
                    <div
                    ...
                    >
                      <img
                        src={iconCard}
                        alt="Icone Caderno"
                        className="notebook-select-card-icon"
                      />
